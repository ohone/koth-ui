...
function MainCard(props: ContentCardProps) {
    ...
    const [cardState, setCardState] = useState<tabNames>(addressMissing ? 'factory' : 'current');

    return (
        <div className='HillCard'>
            <Card style={{ width: '32rem', height:'40rem' }} border="secondary">
            <Card.Header>
                <Tabs defaultActiveKey={cardState} onSelect={k => setCardState(k as tabNames)}>
                    <Tab 
                        eventKey="current"
                        title="Hill" 
...
function getTabProps(tab : tabNames, address : validAddressResult, context: IChainContext) : tabProps {
    const factory : () => factoryProps = () => {return {id: "factory", context: context}};
    const hills : () => hillsProps  = () => {return {id: "hills", context: context}};
